...
void mappers::DevWeightsMapper::registerKeys() {
	registerKeyword("dev_from_building",
	...
	registerKeyword("dev_from_holding",
					[this](const std::string &unused, std::istream &theStream) {
						const commonItems::singleDouble devDbl(theStream);
						devFromHolding = devDbl.getDouble();
					});
	registerKeyword("dev_from_development",
					[this](const std::string &unused, std::istream &theStream) {
						const commonItems::singleDouble devDbl(theStream);
						devFromDev = devDbl.getDouble();
					});
	registerKeyword("dev_treshold",
					[this](std::istream &theStream) { devTreshold = commonItems::getDouble(theStream); });
