...
void mappers::CultureDefinitionsMapper::loadDefinitionsFromEU4Installation(const Configuration &theConfiguration) {
	...
	for (const auto &cultureFile:
		 commonItems::GetAllFilesInFolder(theConfiguration.getEU4Path() / "common/cultures/")) parseFile(
		theConfiguration.getEU4Path() / "common/cultures" / cultureFile);

	for (const auto &cultureFile: commonItems::GetAllFilesInFolder("blankMod/output/common/cultures/")) parseFile(
		"blankMod/output/common/cultures" / cultureFile);
...
void mappers::CultureDefinitionsMapper::registerKeys() {
	registerRegex(R"([\w_]+)",
				  [this](std::string cultureGroupName, std::istream &theStream) {
					  ...
					  if (cultureGroupsMap.contains(cultureGroupName)) {
						  for (const auto &[cultureName, culture]: newGroup->getCultures()) cultureGroupsMap[
							  cultureGroupName]->mergeCulture(cultureName, culture);
					  ...
					  } else { cultureGroupsMap.emplace(cultureGroupName, newGroup); }
				  });
...
void mappers::CultureDefinitionsMapper::buildDefinitions(const CultureMapper &cultureMapper) {
	...
	for (const auto &culture: cultureMapper.getCK3Cultures()) {
		if (!culture->isDynamic() || culture->isEU4Ready()) continue;
		...
		for (const auto &nameList: culture->getNameLists()) {
			...
			if (eu4CultureNameMatch) {
				...
				if (eu4Culture) assignNamesToCulture(eu4Culture, newDefinition, *eu4CultureNameMatch);
				else Log(LogLevel::Warning) << "Cannot find EU4 culture definitions for culture " << *
						eu4CultureNameMatch;
			} else { Log(LogLevel::Warning) << "Cannot find EU4 culture match for namelist " << nameList; }
		}
...
void mappers::CultureDefinitionsMapper::assignCultureToCultureGroup(const std::shared_ptr<CK3::Culture> &sourceCulture,
																	const std::shared_ptr<CultureDefinition> &
																	targetDefinition) {
	// Locate culture grup.
	if (!sourceCulture->getHeritage().empty()) {
		...
		if (cultureGroupMatch) {
			...
			Log(LogLevel::Warning) << "No culture group for culture " << sourceCulture->getName() <<
					", shoving into lost.";
...
std::shared_ptr<mappers::CultureDefinition> mappers::CultureDefinitionsMapper::getCulture(
	const std::string &cultureName) const {
	for (const auto &cultureGroup: cultureGroupsMap | std::views::values)
		if (cultureGroup->containsCulture(cultureName)) return cultureGroup->getCulture(cultureName);
	return nullptr;
}

std::shared_ptr<mappers::CultureGroupDefinition>
mappers::CultureDefinitionsMapper::getCultureGroup(const std::string &cultureGroupName) {
	if (cultureGroupsMap.contains(cultureGroupName)) { return cultureGroupsMap.at(cultureGroupName); } else {
...
std::shared_ptr<mappers::CultureGroupDefinition> mappers::CultureDefinitionsMapper::getCultureGroupForCultureName(
	const std::string &cultureName) const {
	for (const auto &cultureGroup: cultureGroupsMap | std::views::values)
		if (cultureGroup->getCultures().contains(cultureName)) return cultureGroup;
	return nullptr;
}