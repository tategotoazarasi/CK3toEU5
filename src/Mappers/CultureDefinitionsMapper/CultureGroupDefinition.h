...
namespace mappers {
	...
	public:
		...
		[[nodiscard]] auto containsCulture(const std::string &cultureName) const {
			return cultures.contains(cultureName);
		}

		void addMaleNames(const std::vector<std::string> &names) {
			maleNames.insert(maleNames.end(), names.begin(), names.end());
		}

		void addFemaleNames(const std::vector<std::string> &names) {
			femaleNames.insert(femaleNames.end(), names.begin(), names.end());
		}

		void addDynastyNames(const std::vector<std::string> &names) {
			dynastyNames.insert(dynastyNames.end(), names.begin(), names.end());
		}
