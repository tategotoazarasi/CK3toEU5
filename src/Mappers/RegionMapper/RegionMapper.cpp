...
void mappers::RegionMapper::loadRegions(std::istream &areaStream,
										std::istream &regionStream,
										std::istream &superRegionStream) {
	registerAreaKeys();
	parseStream(areaStream);
	clearRegisteredKeywords();
...
void mappers::RegionMapper::registerAreaKeys() {
	registerRegex(R"([\w_]+)",
				  [this](const std::string &areaName, std::istream &theStream) {
...
void mappers::RegionMapper::registerSuperRegionKeys() {
	registerRegex(R"([\w_]+)",
				  [this](const std::string &sregionName, std::istream &theStream) {
...
void mappers::RegionMapper::registerRegionKeys() {
	registerRegex(R"([\w_]+)",
				  [this](const std::string &regionName, std::istream &theStream) {
...
std::optional<std::string> mappers::RegionMapper::getParentAreaName(const int provinceID) const {
	for (const auto &area: areas) { if (area.second->areaContainsProvince(provinceID)) return area.first; }
...
std::optional<std::string> mappers::RegionMapper::getParentRegionName(const int provinceID) const {
	for (const auto &region: regions) { if (region.second->regionContainsProvince(provinceID)) return region.first; }
		...
		for (const auto &requiredRegion: srsRegions) {
			...
			if (regionItr != regions.end()) {
				...
				throw std::runtime_error(
					"Superregion's " + superRegion.first + " region " + requiredRegion.first + " does not exist!");
			}
		}
	}
}
...
void mappers::RegionMapper::linkRegions() {
	for (const auto &region: regions) {
		...
		for (const auto &requiredArea: rareas) {
			...
			if (areaItr != areas.end()) { region.second->linkArea(std::pair(areaItr->first, areaItr->second)); } else {
				throw std::runtime_error(
					"Region's " + region.first + " area " + requiredArea.first + " does not exist!");
			}
		}
	}
}
	...
	for (const auto &area: areas) {
		...
		for (const auto &requiredProvince: rprovinces) {
			...
			if (provinceItr != theProvinces.end()) {
				...
				throw std::runtime_error(
					"Area's " + area.first + " province " + std::to_string(requiredProvince.first) +
					" does not exist!");
			}
		}
	}
