...
void mappers::LocalizationMapper::scrapeLanguage(const std::string &language, const std::filesystem::path &path) {
	if (!commonItems::DoesFolderExist(path / language)) return;
	...
	for (const auto &file: fileNames) {
		...
		if (fileStream.is_open()) scrapeStream(fileStream, language);
		fileStream.close();
	}
}
	...
	for (const auto &lang: std::vector<std::string>{"english", "french", "spanish", "german"}) {
		...
		if (loc.find('$') != std::string::npos) // TODO: handle escaped \$
		{
			...
			if (nestedKey.find('|') != std::string::npos) {
				nestedKey = getLeadStr(nestedKey, 1, "|");
				// chop off localization specifics within the nested key. We can't handle them anyway.
			}
...
const std::string
mappers::LocalizationMapper::selectLanguage(const std::string &language, const LocBlock &block) const {
	if (language == "english") return block.english;
	else if (language == "french") return block.french;
	else if (language == "spanish") return block.spanish;
	else if (language == "german") return block.german;
	else if (language == "korean") return block.korean;
	else if (language == "russian") return block.russian;
	else if (language == "simp_chinese") return block.simp_chinese;
	else return block.english;
}

void mappers::LocalizationMapper::assignSelectLanguage(const std::string &str,
													   const std::string &language,
													   LocBlock &         block) const {
	if (language == "english") block.english = commonItems::remQuotes(str);
	else if (language == "french") block.french = commonItems::remQuotes(str);
	else if (language == "spanish") block.spanish = commonItems::remQuotes(str);
	else if (language == "german") block.german = commonItems::remQuotes(str);
	else if (language == "korean") block.korean = commonItems::remQuotes(str);
	else if (language == "russian") block.russian = commonItems::remQuotes(str);
	else if (language == "simp_chinese") block.simp_chinese = commonItems::remQuotes(str);
	else throw std::invalid_argument(language + " is not currently supported or has a typo.");
	return;
}
	...
	while (!theStream.eof()) {
		...
		if (!key.empty() && !value.empty()) {
			if (localizations.contains(key)) {
				if (language == "english") localizations[key].english = value;
				if (language == "french") localizations[key].french = value;
				if (language == "german") localizations[key].german = value;
				if (language == "korean") localizations[key].korean = value;
				if (language == "russian") localizations[key].russian = value;
				if (language == "simp_chinese") localizations[key].simp_chinese = value;
				if (language == "spanish") localizations[key].spanish = value;
			} else {
				LocBlock newBlock;
				if (language == "english") newBlock.english = value;
				if (language == "french") newBlock.french = value;
				if (language == "german") newBlock.german = value;
				if (language == "korean") newBlock.korean = value;
				if (language == "russian") newBlock.russian = value;
				if (language == "simp_chinese") newBlock.simp_chinese = value;
				if (language == "spanish") newBlock.spanish = value;
...
std::pair<std::string, std::string> mappers::LocalizationMapper::determineKeyLocalizationPair(const std::string &text) {
	...
	if (text.size() > 2 && (text[0] == '#' || text[1] == '#')) return blankReturn;

	const auto keyBeginPos = text.find_first_not_of(' ');
	if (keyBeginPos == std::string::npos) return blankReturn;

	const auto keyEndPos = text.find_first_of(':', keyBeginPos + 1);
	const auto quotePos  = text.find_first_of('"', keyEndPos);
	if (quotePos == std::string::npos) return blankReturn;

	const auto localizationBeginPos = quotePos + 1;
	const auto localizationEndPos   = text.find_last_of('"', text.size());
	return std::make_pair(text.substr(keyBeginPos, keyEndPos - keyBeginPos),
						  text.substr(localizationBeginPos, localizationEndPos - localizationBeginPos));
}
...
std::optional<mappers::LocBlock> mappers::LocalizationMapper::getLocBlockForKey(const std::string &key) const {
	...
	if (!keyItr->second.english.empty() && (keyItr->second.spanish.empty() || keyItr->second.german.empty() || keyItr->
		second.french.empty() ||
		...
		if (newBlock.korean.empty()) newBlock.korean = newBlock.english;
		if (newBlock.russian.empty()) newBlock.russian = newBlock.english;
		if (newBlock.simp_chinese.empty()) newBlock.simp_chinese = newBlock.english;
		return newBlock;
	}
	...
	if (keyItr->second.english.empty()) return std::nullopt;

	return keyItr->second;
}

std::optional<std::string>
mappers::LocalizationMapper::reverseLookupCultureName(const std::string &localization) const {
	// This looks for specifically *_name keys cultures use.

	for (const auto &[locName, locBlock]: localizations) {
		if (locBlock.english == localization || locBlock.french == localization || locBlock.german == localization ||
			locBlock.korean == localization ||
			locBlock.russian == localization || locBlock.simp_chinese == localization || locBlock.spanish ==
			localization)
			if (locName.find("_name") != std::string::npos) return locName;
	}
	return std::nullopt;
}
...
std::string mappers::getLeadStr(const std::string &str, const int occurrence, const std::string &match) {
	if (const auto &i = str.find(match); i != std::string::npos)
		if (occurrence == 1) return str.substr(0, i);
		else return str.substr(0, i) + match + getLeadStr(str.substr(i + match.length()), occurrence - 1, match);
	else return str;
}
...
std::string mappers::getTailStr(const std::string &str, const int occurrence, const std::string &match) {
	if (const auto &i = str.find(match); i != std::string::npos)
		if (occurrence == 1) {
			...
			if (tail.starts_with("!")) tail = tail.substr(1, tail.length());
			return tail;
		} else return getTailStr(str.substr(i + match.length()), occurrence - 1, match);
	else return str;
}
...
std::string mappers::cleanLocMarkups(const std::string &loc) {
	...
	while (pos != std::string::npos) {
		...
		if (pos <= workingLoc.size() - 2 && workingLoc.at(pos + 1) == '!') {
			...
			if (endpos == std::string::npos) return loc; // bail
