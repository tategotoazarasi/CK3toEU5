...
void mappers::ProvinceMapper::registerKeys() {
	registerRegex(R"([0-9\.]+)",
				  [this](const std::string &unused, std::istream &theStream) {
					  ...
					  if (mappings.empty()) { mappings = ProvinceMappingsVersion(theStream).getMappings(); } else {
...
void mappers::ProvinceMapper::createMappings() {
	for (const auto &mapping: mappings) {
		...
		if (mapping->getCK3Provinces().empty()) continue;
		if (mapping->getEU4Provinces().empty()) continue;

		for (auto ck3Number: mapping->getCK3Provinces()) {
			if (ck3Number) CK3ToEU4ProvinceMap.insert(std::make_pair(ck3Number, mapping->getEU4Provinces()));
		}
		for (const auto &eu4Number: mapping->getEU4Provinces()) {
			if (eu4Number) EU4ToCK3ProvinceMap.insert(std::make_pair(eu4Number, mapping->getCK3Provinces()));
		}
	}
}
...
std::vector<int> mappers::ProvinceMapper::getEU4ProvinceNumbers(const std::string &ck3CountyName) const {
	...
	if (mapping != countiesToBaronies.end()) {
		...
		if (remapping != CK3ToEU4ProvinceMap.end()) { return remapping->second; } else {
			Log(LogLevel::Warning) << "Province Mapper mismatch - " << ck3CountyName << " with baronyID " << mapping->
					second << " has no mappings!";
		}
	} else if (ck3CountyName.starts_with("c_nf_")) {
		// Nothing, false alarm, junk dynamic title.
	} else { Log(LogLevel::Warning) << "Province Mapper mismatch - " << ck3CountyName << " has no mappings!"; }
...
void mappers::ProvinceMapper::transliterateMappings(std::map<std::string, std::shared_ptr<CK3::Title> > titles) {
	...
	for (const auto &title: titles) {
		if (title.second->getLevel() == CK3::LEVEL::BARONY) {
			...
			auto countyTitle = std::pair(title.second->getDJLiege()->second->getName(),
										 title.second->getDJLiege()->second);
			// Is the county sane?
			if (countyTitle.second->getLevel() != CK3::LEVEL::COUNTY || !countyTitle.second->getClay() || !countyTitle.
				second->getClay()->getCounty()) {
	...
	for (const auto &mapping: mappings) {
		for (auto ck3ID: mapping->getCK3Provinces()) {
			if (theMatrix.count(ck3ID)) {
				...
				Log(LogLevel::Error) << "Province mapping ck3 = " + std::to_string(ck3ID) +
						" is invalid. We cannot find related county.";
			}
		}
	}
