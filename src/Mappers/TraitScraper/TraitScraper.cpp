...
void mappers::TraitScraper::registerKeys() {
	...
	registerRegex(commonItems::catchallRegex,
				  [this](const std::string &traitName, std::istream &theStream) {
					  ...
					  if (newScraping.getIndex() > 0) // indexes start at 1!
					  {
						  ...
						  } else { traits.emplace(newScraping.getIndex(), traitName); }
					  } else // otherwise index them ourselves.
					  {
						  if (!traits.contains(index)) { traits.emplace(index, traitName); } else {
...
std::optional<std::string> mappers::TraitScraper::getTraitForID(int ID) const {
	...
	if (traitsItr != traits.end()) return traitsItr->second;
	return std::nullopt;
}