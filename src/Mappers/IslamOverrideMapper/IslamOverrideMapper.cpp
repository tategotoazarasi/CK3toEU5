...
void mappers::IslamOverrideMapper::registerKeys() {
	registerKeyword("link",
					[this](std::istream &theStream) {
						...
						if (newMapping.getReplacementMapping().first && !newMapping.getReplacementMapping().second.
							empty()) replacements.emplace(newMapping.getReplacementMapping());
					});
...
std::optional<std::string> mappers::IslamOverrideMapper::getReplacementReligionForProvince(int provinceID) const {
	...
	if (religion == replacements.end()) return std::nullopt;
	else return religion->second;
}