...
bool mappers::GovernmentsMapping::matchGovernment(const std::string &ck3Government, const std::string &incTitle) const {
	if (!ck3title.empty() && ck3title != incTitle) return false; // these are reserved for their recipients only.
	if (!ck3title.empty() && ck3title == incTitle) return true;
	if (ck3governments.count(ck3Government)) return true;
	return false;
}