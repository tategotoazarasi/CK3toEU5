...
std::optional<std::pair<std::string, std::string> > mappers::GovernmentsMapper::matchGovernment(
	const std::string &ck3Government,
	...
	for (const auto &mapping: govMappings) {
		if (mapping.getCK3Title().empty()) continue;
