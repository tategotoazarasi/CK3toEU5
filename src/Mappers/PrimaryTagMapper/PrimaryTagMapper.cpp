...
void mappers::PrimaryTagMapper::loadPrimaryTags(const Configuration &theConfiguration) {
	...
	for (const auto &filename: commonItems::GetAllFilesInFolder("blankMod/output/common/cultures/")) parseFile(
		"blankMod/output/common/cultures" / filename);
	for (const auto &filename:
		 commonItems::GetAllFilesInFolder(theConfiguration.getEU4Path() / "common/cultures/")) parseFile(
		theConfiguration.getEU4Path() / "common/cultures" / filename);
...
std::optional<std::string> mappers::PrimaryTagMapper::getPrimaryTagForCulture(const std::string &culture) const {
	...
	if (cultureItr != cultureTags.end()) return cultureItr->second;
	return std::nullopt;
}