...
void mappers::RulerPersonalitiesMapper::registerKeys() {
	registerRegex("[a-zA-Z0-9_-]+",
				  [this](const std::string &personality, std::istream &theStream) {
...
std::set<std::string> mappers::RulerPersonalitiesMapper::evaluatePersonalities(
	const std::shared_ptr<CK3::Character> &theCharacter) const {
	...
	for (const auto &trait: incTraits) ck3Traits.insert(trait.second);
