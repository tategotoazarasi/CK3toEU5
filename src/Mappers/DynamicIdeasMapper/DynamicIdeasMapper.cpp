...
mappers::DynamicIdeasMapper::DynamicIdeasMapper(const LocalizationMapper &localizationMapper) : locs(
	localizationMapper) {
	registerKeys();
...
mappers::DynamicIdeasMapper::DynamicIdeasMapper(std::istream &            theStream,
												const LocalizationMapper &localizationMapper) : locs(
	localizationMapper) {
	registerKeys();
...
void mappers::DynamicIdeasMapper::registerKeys() {
	registerKeyword("link",
					[this](std::istream &theStream) {
						...
						{
							if (auto possibleTradition = scraper.getTradition(); possibleTradition) traditionMap.
									emplace(possibleTradition.value(), effects);
							else if (auto possibleEthos = scraper.getEthos(); possibleEthos) ethosMap.emplace(
								possibleEthos.value(),
								effects);
							...
							} else Log(LogLevel::Warning) << "A link was skipped due to invalid input.";
						} else // Is a rule based override mapping
						{
							if (auto possibleTradition = scraper.getTradition(); possibleTradition) rules.emplace(
								DynamicIdeasRule(newRules, effects, possibleTradition.value(), scraper.getIdeaName()));
							else if (auto possibleEthos = scraper.getEthos(); possibleEthos) rules.emplace(
								DynamicIdeasRule(newRules, effects, possibleEthos.value(), scraper.getIdeaName()));
						}
					});
...
void mappers::DynamicIdeasMapper::processRules() {
	...
	for (const auto &rule: rules)
		if (rule.getReplacee().find("tradition") != std::string::npos) traditionMap.emplace(
			rule.getReplacement(),
			rule.getNewEffect());
