...
mappers::DynamicIdeasRule::DynamicIdeasRule(const std::vector<RulePair> &    ruleInfo,
											...
											const std::optional<std::string> ideaName) : ruleInfo(ruleInfo),
	replacee(replacee),
	newEffect(newEffect),
	replacementIdentifier(replacee) {
	if (ideaName) replacementIdentifier = ideaName.value();
	else replacementIdentifier          += '_'; // Need extra _ to know when non-base tradition name starts
	...
	for (const auto &rule: ruleInfo) {
		...
		if (!ideaName) {
			...
			if (rule.value.find(prefix) != std::string::npos) replacementIdentifier += ("_" + rule.value);
			else replacementIdentifier += ("_" + ruleName + "_" + rule.value);
		}
	}
}
...
bool mappers::DynamicIdeasRule::testRules(const std::shared_ptr<CK3::Culture> culture) const {
	...
	for (const auto &rule: ruleInfo) {
		...
		if (!testRule(rule.rule_type, rule.value, culture)) return false;
	}
	return true;
}
...
bool mappers::DynamicIdeasRule::operator<(const DynamicIdeasRule &rhs) const {
	...
	if (precedenceLevel == rhs.precedenceLevel) {
		if (ruleInfo == rhs.ruleInfo) return replacee != rhs.replacee;
		else return true;
	}
	...
	if (lComparator.empty()) return false;
	...
	if (rComparator.empty()) return true;
...
bool mappers::DynamicIdeasRule::operator==(const DynamicIdeasRule &rhs) const {
	if (replacee != rhs.replacee) return false;
	if (replacementIdentifier != rhs.replacementIdentifier) return false;
	if (precedenceLevel != rhs.precedenceLevel) return false;
	if (newEffect != rhs.newEffect) return false;
...
bool mappers::DynamicIdeasRule::testRule(const RULE_TYPE                     ruleType,
										 const std::string &                 ruleValue,
										 const std::shared_ptr<CK3::Culture> culture) const {
	switch (ruleType) {
		case RULE_TYPE::HERITAGE: return culture->getHeritage() == ruleValue;
		case RULE_TYPE::ETHOS_TRIGGER: return culture->getEthos() == ruleValue;
...
std::ostream &mappers::operator<<(std::ostream &output, const DynamicIdeasRule &rule) {
	...
	for (auto &pred: rule.getPrecedence()) output << " " + std::to_string(pred);
	output << " } Precedence\n";
