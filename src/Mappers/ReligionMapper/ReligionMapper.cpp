...
std::optional<std::string> mappers::ReligionMapper::getEU4ReligionForCK3Religion(
	const std::string &ck3Religion,
	const std::string &ck3ReligiousHead) const {
	if (const auto &mapping = ReligionHeadToEU4ReligionMap.find(ck3ReligiousHead); mapping !=
		ReligionHeadToEU4ReligionMap.end()) return ReligionHeadToEU4ReligionMap.find(ck3ReligiousHead)->second.
			eu4religion;
	if (const auto &mapping = CK3ToEU4ReligionMap.find(ck3Religion); mapping != CK3ToEU4ReligionMap.end()) return
			mapping->second.eu4religion;
	return std::nullopt;
}

std::optional<std::string> mappers::ReligionMapper::getEU4SchoolForCK3Religion(const std::string &ck3Religion) const {
	if (const auto &mapping = CK3ToEU4ReligionMap.find(ck3Religion); mapping != CK3ToEU4ReligionMap.end()) return
			mapping->second.eu4school;
	return std::nullopt;
}
...
void mappers::ReligionMapper::importCK3Faiths(const CK3::Faiths &         faiths,
	...
	for (const auto &faith: faiths.getFaiths()) {
		...
		} else if (getEU4ReligionForCK3Religion(faith.second->getName(), faith.second->getReligiousHead()) ==
			"west_african_pagan" &&
		...
		} else if (faith.second->getReformedFlag())
		// This is for unreformed religions that have been reformed (Game only stores this info in the old religon)
		{
			const auto &displayName = faith.second->getCustomAdj(); // Catholic, not catholicism
			LocBlock    locBlock;
			locBlock.english = displayName;
			// CK3 already calls it "Old Religion", this will just overwrite the default EU4 localization calling it something
			// else.
...
void mappers::ReligionMapper::importCK3Faith(const CK3::Faith &          faith,
	...
	locBlock.german  = displayName;
	locBlock.spanish = displayName;
	// Ck3 save only stores the one display name, so we have no choice but to copy it around.
	...
	if (!description.empty()) {
		...
		const auto &match = localizationMapper.getLocBlockForKey(faith.getTemplate());
		// We'll attempt to copy over parent desc.
		if (match) {
	...
	{
		const auto &dstReligion = getEU4ReligionForCK3Religion(faith.getTemplate(), faith.getReligiousHead());
		// and this would map into eu4's "adamites"
		if (dstReligion) {
			eu4ParentReligion = *dstReligion;
			const auto &match = religionDefinitionMapper.getStaticBlob(*dstReligion);
			if (match) { staticBlob = *match; } else {
				Log(LogLevel::Warning) << "EU4 Religion Template " << *dstReligion <<
						" does not exist! Check religion_globals.txt!";
			}
		} else {
		...
		for (const auto &similarFaith: ck3ReligionScraper.getSimilarFaiths(origName)) {
			if (const auto &dstReligion = getEU4ReligionForCK3Religion(similarFaith, faith.getReligiousHead());
				dstReligion) {
				eu4ParentReligion = *dstReligion;
				Log(LogLevel::Warning) << origName <<
						" is unknown to the converter. We found a \"suitable\" replacement with: " << similarFaith <<
						"->"
						<< eu4ParentReligion << ". Overriding.";
				if (const auto &match = religionDefinitionMapper.getStaticBlob(*dstReligion); match) staticBlob = *
						match;
				break;
			}
		}
		if (eu4ParentReligion.empty()) Log(LogLevel::Warning) << "CK3 Religion Template for " << origName <<
				" does not exist! We will be scraping defaults.";
	}
	...
	for (const auto &doctrine: faith.getDoctrines()) {
		...
		if (match) {
			if (!match->getAllowedConversion().empty()) allowedConversion += match->getAllowedConversion() + "\n";
			if (!match->getCountry().empty() && countryCount < 4)
			// This should prevent religions from having too many modifiers at once
			{
			...
			if (!match->getProvince().empty()) province += match->getProvince() + "\n";
			if (!match->getUnique().empty()) {
				if (match->getUnique().find("personal_deity = yes") != std::string::npos) {
					if (eu4ParentReligion.find("germanic_pagan") != std::string::npos || eu4ParentReligion.
						find("norse_pagan") != std::string::npos) {
						deityCount++;
				...
				} else unique = match->getUnique(); // overriding uniques, always.
			}
			if (!match->getNonUnique().empty()) nonUnique += match->getNonUnique() + "\n";
		} else {
			Log(LogLevel::Warning) << "Doctrine  definition for " << doctrine <<
					" does not exist! Check doctrine_definitions.txt!";
		}
	}
	...
	if (unique.find("personal_deity = yes") != std::string::npos) newReligion.personalDeityNumber = deityCount;
	...
	if (match) { newReligion.religionGroup = *match; } else {
...
void mappers::ReligionMapper::createPersonalDeities(const std::string &parentReligion, const short deityCount) {
	...
	if (parentReligion.find("germanic_pagan") != std::string::npos) {
		...
		descBlock.english  =
				"Odin is the major god of the Norse pantheon, commonly associated with war, victory and death.";
		descBlock.french =
				"Odin est le dieu principal du pantheon norrois, et est souvent associe a la guerre, la victoire et la mort.";
		descBlock.german =
				"Odin ist der Hauptgott der nordischen Ruhmeshalle und wird allgemein mit Krieg, Sieg und Tod assoziiert.";
		descBlock.spanish =
				"Odin es el dios principal del Panteon nordico, generalmente asociado con la guerra, la victoria y la muerte.";
		...
		descBlock.english  =
				"Freyja is the goddess commonly worshipped for her love, beauty, fertility and her promises of wealth.";
		descBlock.french =
				"Freya est une deesse souvent veneree pour l'amour, la beaute, la fertilite et ses promesses de richesse.";
		descBlock.german =
				"Freya ist die allgemein fur ihre Liebe, Schonheit, Fruchtbarkeit und ihre Versprechen des Wohlstands verehrte Gottin.";
		descBlock.spanish =
				"Freya es la diosa a la que se suele adorar por su amor, belleza, fertilidad y sus promesas de la riqueza.";
		...
		descBlock.english  =
				"Tor is the son of Odin and is the god protecting humankind against the terrors trying to enter Midgard.";
		descBlock.french =
				"Thor est le fils d'Odin; il protege l'humanite contre les demons tentant de penetrer Midgard.";
		descBlock.german =
				"Thor ist der Sohn von Odin und der Gott, der die Menschheit vor den Schrecken beschutzt, die versuchen, in Midgard einzufallen.";
		descBlock.spanish =
				"Thor es hijo de Odin y y es el dios que protege la especie humana contra los terrores que tratan de entrar en Midgard.";
		...
		descBlock.english  =
				"The father of Freya, god of the sea and wealth will bring food to the table for his worshippers.";
		descBlock.french =
				"Pere de Freya, le dieu de la mer et des richesses amene de la nourriture a la table de ceux qui le venerent.";
		descBlock.german =
				"Der Vater von Freya, Gott des Meeres und des Wohlstands, deckt den Tisch seiner Verehrer mit Nahrung ein.";
		...
		descBlock.french   =
				"Snotra est une deesse avisee; en son honneur, un homme ou une femme sage peut etre qualifie de \'snotr\'.";
		descBlock.german =
				"Snotra ist die Gottin der Klugheit. Ein weiser Mann oder eine weise Frau kann ihr zu Ehren als snotr bezeichnet werden.";
		descBlock.spanish =
				"Snotra es la diosa de la inteligencia. Un hombre o una mujer sabia puede ser llamada snotr en su honor.";
