...
void mappers::CultureMappingRule::registerKeys() {
	...
	registerKeyword("province",
					[this](const std::string &unused, std::istream &theStream) {
						...
						try { provinces.insert(stoi(provinceStr.getString())); } catch (std::exception &) {
...
std::optional<std::string> mappers::CultureMappingRule::cultureMatch(const std::string &ck3culture,
	...
	if (ck3culture.empty()) return std::nullopt;

	if (!cultures.contains(ck3culture)) return std::nullopt;

	if (!owners.empty())
		if (eu4ownerTag.empty() || !owners.contains(eu4ownerTag)) return std::nullopt;

	if (!religions.empty())
		if (eu4religion.empty() || !religions.contains(eu4religion)) return std::nullopt;

	// This is a straight province check, not regions.
	if (!provinces.empty() && regions.empty())
		if (!eu4Province || !provinces.contains(eu4Province)) return std::nullopt;

	// Asking for a regions check without an incoming province is pointless.
	if (!regions.empty() && !eu4Province) return std::nullopt;
	...
	if (eu4Province && !regions.empty()) {
		...
		for (const auto &region: regions) {
			if (!regionMapper->regionNameIsValid(region)) {
				Log(LogLevel::Warning) << "Checking for culture " << ck3culture << " inside invalid region: " << region
		...
		if (!provinces.empty() && provinces.count(eu4Province)) regionMatch = true;
