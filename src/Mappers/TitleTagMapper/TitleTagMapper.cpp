	...
	registerKeyword("link",
					[this](const std::string &unused, std::istream &theStream) {
						...
						if (mapping.getEU4Tag().empty()) {
							Log(LogLevel::Warning) << "Issue with Title-tag mapping " << mapping.getCK3Title() <<
									" - no eu4 tag? Skipping.";
						} else { theMappings.emplace_back(mapping); }
					});
...
void mappers::TitleTagMapper::registerChineseKeys() {
	registerKeyword("link",
					[this](const std::string &unused, std::istream &theStream) {
						auto mapping = TitleTagMapping(theStream);
						if (mapping.getEU4Tag().empty()) {
							Log(LogLevel::Warning) << "Issue with chinese Title-tag mapping " << mapping.getCK3Title()
									<< " - no eu4 tag? Skipping.";
						} else { chineseMappings.emplace_back(mapping); }
					});
	registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);
}
...
std::optional<std::string> mappers::TitleTagMapper::getTagForTitle(const std::string &ck3Title, int eu4Capital) {
	...
	// not an option!
	if (ck3Title.empty()) return std::nullopt;
...
std::string mappers::TitleTagMapper::generateNewTag() {
	...
	if (generatedEU4TagSuffix == '[') // [, 1 past Z
	{
		...
		if (generatedEU4TagMidfix == ':') {
			...
			while (reservedPrefixes.contains(generatedEU4TagPrefix)) --generatedEU4TagPrefix;
		}
	}
	return eu4Tag;
}
...
std::optional<std::string> mappers::TitleTagMapper::getChinaForTitle(const std::string &ck3Title) const {
	...
	for (const auto &mapping: chineseMappings) {
		if (const auto &match = mapping.titleMatch(ck3Title); match) { return *match; }
	}

	// Try for fallback
	for (const auto &mapping: chineseMappings) {
		if (const auto &match = mapping.fallbackMatch(); match) { return mapping.getEU4Tag(); }
	}
...
std::set<std::string> mappers::TitleTagMapper::getAllChinas() const {
	...
	for (const auto &mapping: chineseMappings) toReturn.insert(mapping.getEU4Tag());
	return toReturn;
}