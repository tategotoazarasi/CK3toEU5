...
mappers::ReligionGroupScraper::ReligionGroupScraper() {
	...
	registerKeys();
	for (const auto &fileName: commonItems::GetAllFilesInFolder("blankmod/output/common/religions/")) parseFile(
		"blankmod/output/common/religions" / fileName);
...
void mappers::ReligionGroupScraper::registerKeys() {
	registerRegex(commonItems::catchallRegex,
				  [this](const std::string &religionGroupName, std::istream &theStream) {
...
mappers::ReligionGroupScraper::getReligionGroupForReligion(const std::string &religion) const {
	for (const auto &religionGroupItr: religionGroups)
		if (religionGroupItr.second.count(religion)) return religionGroupItr.first;
	return std::nullopt;
}