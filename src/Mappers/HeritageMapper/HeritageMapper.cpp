	...
	registerKeyword("link",
					[this](std::istream &theStream) {
						...
						if (!theMapping.getEU4CultureGroup().empty() && !theMapping.getCK3Heritages().empty())
							for (const auto &heritage: theMapping.getCK3Heritages()) heritagesToCultures.emplace(
								heritage,
								theMapping.getEU4CultureGroup());
					});
	registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);
}

std::optional<std::string> mappers::HeritageMapper::getCultureGroupForHeritage(const std::string &heritage) const {
	if (heritagesToCultures.contains(heritage)) return heritagesToCultures.at(heritage);
