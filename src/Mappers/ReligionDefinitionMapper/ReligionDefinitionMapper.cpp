...
void mappers::ReligionDefinitionMapper::registerKeys() {
	registerKeyword("current_icons",
					[this](const std::string &definition, std::istream &theStream) {
...
void mappers::ReligionDefinitionMapper::registerStaticKeys() {
	registerRegex(commonItems::catchallRegex,
				  [this](const std::string &religionName, std::istream &theStream) {
					  const auto blob = commonItems::stringOfItem(theStream).getString();
					  if (blob.size() > 4) staticStorage.insert(
						  std::make_pair(religionName, blob.substr(3, blob.size() - 4)));
					  else staticStorage.clear();
				  });
}

std::optional<mappers::ReligionDefinitionMapping> mappers::ReligionDefinitionMapper::getDefinition(
	const std::string &definition) const {
	const auto &mapping = mappings.find(definition);
	if (mapping != mappings.end()) return mapping->second;
	return std::nullopt;
}
...
std::optional<std::string> mappers::ReligionDefinitionMapper::getStaticBlob(const std::string &religion) const {
	...
	if (mapping != staticStorage.end()) return mapping->second;
	return std::nullopt;
}