...
void CK3::Religions::linkFaiths(const Faiths &theFaiths) {
	...
	for (const auto &religion: religions) {
		...
		for (const auto &faith: religionFaiths) {
			...
			if (faithDataItr != faithData.end()) { replacementMap.insert(*faithDataItr); } else {
				throw std::runtime_error(
					"Religion " + religion.second->getName() + " has faith " + std::to_string(faith.first) +
