...
namespace CK3 {
	...
	public:
		...
		void loadReligion(const std::pair<long long, std::shared_ptr<Religion> > &theReligion) {
	...
	private:
		...
		doctrines;
		// This is a vector in order to keep order consistent. We want the first things read (tenets) to be the first things output, ALWAYS
