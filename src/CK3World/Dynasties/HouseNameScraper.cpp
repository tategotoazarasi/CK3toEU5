...
void CK3::HouseNameScraper::registerKeys() {
	registerRegex(commonItems::catchallRegex,
				  [this](const std::string &houseKey, std::istream &theStream) {
...
std::optional<std::string> CK3::HouseNameScraper::getNameForKey(const std::string &houseKey) const {
	if (houseNames.contains(houseKey)) return houseNames.at(houseKey).getName();
	return std::nullopt;
}

std::optional<std::string> CK3::HouseNameScraper::getPrefixForKey(const std::string &houseKey) const {
	if (houseNames.contains(houseKey)) return houseNames.at(houseKey).getPrefix();
	return std::nullopt;
}