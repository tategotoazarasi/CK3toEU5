...
void CK3::Dynasties::registerKeys() {
	registerKeyword("dynasty_house",
					[this](const std::string &unused, std::istream &theStream) { houses = Houses(theStream); });
	registerKeyword("dynasties",
					[this](const std::string &unused, std::istream &theStream) {
						dynasties = Dynasties(theStream).getDynasties();
					});
	registerRegex(R"(\d+)",
				  [this](const std::string &gameID, std::istream &theStream) {
...
void CK3::Dynasties::linkCoats(const CoatsOfArms &coats) {
	...
	for (const auto &dynasty: dynasties) {
		if (!dynasty.second->getCoA()) continue;
		...
		if (coatDataItr != coatData.end()) {
			...
			Log(LogLevel::Warning) << "Dynasty " + std::to_string(dynasty.first) + " has CoA " + std::to_string(
						dynasty.second->getCoA()->first) +
