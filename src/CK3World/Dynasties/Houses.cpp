...
void CK3::Houses::linkDynasties(const Dynasties &dynasties) {
	...
	for (const auto &house: houses) {
		...
		if (dynastyDataItr != dynastyData.end()) {
			...
			Log(LogLevel::Error) << "House " << std::to_string(house.first) << " has dynasty " << std::to_string(
						house.second->getDynasty().first)
...
void CK3::Houses::importNames(const HouseNameScraper &houseNameScraper) {
	...
	for (const auto &house: houses | std::views::values) {
		if (house->getName().empty() && houseNameScraper.getNameForKey(house->getKey()) && !houseNameScraper.
			getNameForKey(house->getKey())->empty()) {
			house->setName(*houseNameScraper.getNameForKey(house->getKey()));
			++nameCounter;
		}
		if (house->getPrefix().empty() && houseNameScraper.getPrefixForKey(house->getKey()) && !houseNameScraper.
			getPrefixForKey(house->getKey())->empty()) {
