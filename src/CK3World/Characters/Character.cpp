...
void CK3::Character::registerKeys() {
	...
	registerKeyword("skill",
					[this](const std::string &unused, std::istream &theStream) {
						...
						} else Log(LogLevel::Error) << "Character " << charID << " has a malformed skills block! Size: "
								<< skillList.size();
					});
	registerKeyword("traits",
					[this](const std::string &unused, std::istream &theStream) {
						for (const auto traitID: commonItems::intList(theStream).getInts()) traits.insert(
							std::pair(traitID, std::string()));
					});
	registerKeyword("alive_data",
					[this](const std::string &unused, std::istream &theStream) {
	...
	registerKeyword("piety",
					[this](const std::string &unused, std::istream &theStream) {
						const auto tempBlock = Character(theStream, charID);
						piety                = tempBlock.getAccumulated();
					});
	registerKeyword("prestige",
					[this](const std::string &unused, std::istream &theStream) {
						const auto tempBlock = Character(theStream, charID);
						prestige             = tempBlock.getAccumulated();
					});
	registerKeyword("accumulated",
					[this](const std::string &unused, std::istream &theStream) {
						accumulated = commonItems::singleDouble(theStream).getDouble();
					});
	registerKeyword("obedience_target",
					[this](const std::string &unused, std::istream &theStream) {
						suzerain = std::make_pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
					});
	registerKeyword("gold",
					[this](const std::string &unused, std::istream &theStream) { goldParser.parseStream(theStream); });
	registerKeyword("court_data",
					[this](const std::string &unused, std::istream &theStream) {
						const auto tempBlock = Character(theStream, charID);
	...
	registerKeyword("landed_data",
					[this](const std::string &unused, std::istream &theStream) {
						characterDomain = CharacterDomain(theStream);
					});
	registerKeyword("family_data",
					[this](const std::string &unused, std::istream &theStream) {
						const auto tempBlock = Character(theStream, charID);
						spouse               = tempBlock.getSpouse();
					});
	registerKeyword("primary_spouse",
					[this](const std::string &unused, std::istream &theStream) {
	...
	registerKeyword("claim",
					[this](const std::string &unused, std::istream &theStream) {
						...
						for (const auto &blob: blobList) {
							...
							if (titleID) claims.insert(std::pair(titleID, nullptr));
						}
					});
	...
	goldParser.registerKeyword("value",
							   [this](const std::string &unused, std::istream &theStream) {
...
void CK3::Character::dropTitleFromDomain(long long titleID) {
	if (characterDomain) {
		...
		for (const auto &title: characterDomain->getDomain()) {
			if (title.first != titleID) replacementDomain.emplace_back(title);
		}
...
bool CK3::Character::hasTrait(const std::string &wantedTrait) const {
	return std::any_of(traits.begin(),
					   traits.end(),
					   [wantedTrait](const std::pair<int, std::string> &trait) { return trait.second == wantedTrait; });
}