...
namespace CK3 {
	...
	public:
		...
		// Used to mark characters (advisors) so they cannot be reused in different tag.
		...
		void loadRealmCapital(const std::pair<long long, std::shared_ptr<Title> > &theRealmCapital) {
			characterDomain->loadRealmCapital(theRealmCapital);
		}

		void loadDomain(const std::vector<std::pair<long long, std::shared_ptr<Title> > > &theDomain) {
			characterDomain->loadDomain(theDomain);
		}

		void loadClaims(const std::map<long long, std::shared_ptr<Title> > &theClaims) { claims = theClaims; }

		void loadEmployer(const std::pair<long long, std::shared_ptr<Character> > &theEmployer) {
			employer = theEmployer;
		}

		void loadSpouse(const std::pair<long long, std::shared_ptr<Character> > &theSpouse) { spouse = theSpouse; }

		void loadSuzerain(const std::pair<long long, std::shared_ptr<Character> > &theSuzerain) {
			suzerain = theSuzerain;
		}
		...
		void loadCouncilors(const std::map<long long, std::shared_ptr<Character> > &newCouncilors) {
			councilors = newCouncilors;
		}
