...
void CK3::Characters::registerKeys() {
	registerRegex(R"(\d+)",
				  [this](const std::string &charID, std::istream &theStream) {
	...
	for (const auto &character: characters) {
		...
		if (cultureDataItr != cultureData.end()) {
			...
			throw std::runtime_error(
				"Character " + std::to_string(character.first) + " has culture " + std::to_string(
					character.second->getCulture()->first) +
...
void CK3::Characters::linkFaiths(const Faiths &faiths) {
	...
	for (const auto &character: characters) {
		...
		if (faithDataItr != faithData.end()) {
			...
			throw std::runtime_error(
				"Character " + std::to_string(character.first) + " has faith " + std::to_string(
					character.second->getFaith()->first) +
				" which has no definition!");
		}
	}
	Log(LogLevel::Info) << "<> " << counter << " characters updated.";
...
void CK3::Characters::linkHouses(const Houses &houses) {
	...
	for (const auto &character: characters) {
		...
		if (houseDataItr != houseData.end()) {
			...
			throw std::runtime_error(
				"Character " + std::to_string(character.first) + " has house " + std::to_string(
					character.second->getHouse().first) + " which has no definition!");
		}
	}
	Log(LogLevel::Info) << "<> " << counter << " characters updated, " << missingCulture << " are missing culture, " <<
			missingFaith << " are missing faith.";
}
...
void CK3::Characters::linkTitles(const Titles &titles) {
	...
	for (const auto &title: titleData) IDCache.insert(std::pair(title.second->getID(), title.second));
	...
	for (const auto &character: characters) {
		...
		if (domain && domain->getRealmCapital().first) {
			...
			if (titleDataItr != IDCache.end()) {
				...
				Log(LogLevel::Warning) << "Character " << character.first << " has capital " << domain->
						getRealmCapital().first << " which has no definition!";
			}
			...
			for (const auto &title: domain->getDomain()) {
				...
				if (titleDataItr != IDCache.end()) {
					...
					throw std::runtime_error(
						"Character " + std::to_string(character.first) + " owns title " + std::to_string(title.first) +
						" which has no definition!");
				}
			}
		...
		for (const auto &claim: character.second->getClaims()) {
			...
			if (titleDataItr != IDCache.end()) {
				...
				Log(LogLevel::Error) << "Character " << std::to_string(character.first) << " claims title " <<
	...
	Log(LogLevel::Info) << "<> " << capitalCounter << " capitals, " << titleCounter << " titles and " << claimCounter <<
			" claims updated.";
}
...
void CK3::Characters::linkCharacters() {
	...
	Log(LogLevel::Info) << "<> " << spouseCounter << " spouses, " << employerCounter << " employers, " <<
			suzerainCounter << " suzerains updated.";
}
