...
void CK3::World::registerKeys(const std::shared_ptr<Configuration> &theConfiguration,
	...
	metaPreParser.registerKeyword("meta_data",
							   ...
							   [this, theConfiguration](std::istream &theStream) {
								   ...
								   for (const auto &mod: mods)
									   if (mod.name == "CoA Designer") {
										   Log(LogLevel::Notice) <<
												   "CoA Designer mod enabed; player CoA will be generated.";
										   coaDesigner = true;
									   }
							   });
	...
	registerKeyword("meta_data",
					[this](std::istream &theStream) {
						if (saveGame.parsedMeta) { commonItems::ignoreItem("unused", theStream); } else {
							metaParser.parseStream(theStream);
							saveGame.parsedMeta = true;
						}
					});
	registerKeyword("currently_played_characters",
					[this](std::istream &theStream) {
						auto playedCharacters = commonItems::getLlongs(theStream);
						if (!playedCharacters.empty()) playerID = playedCharacters[0];
					});
	...
	registerKeyword("bookmark_date",
					[this](const std::string &unused, std::istream &theStream) {
						const commonItems::singleString startDateString(theStream);
						startDate = date(startDateString.getString());
					});
	registerKeyword("version",
					[this, converterVersion](const std::string &unused, std::istream &theStream) {
						...
						if (converterVersion.getMinSource() > CK3Version) {
							Log(LogLevel::Error) << "Converter requires a minimum save from v" << converterVersion.
									getMinSource().toShortString();
						...
						if (!converterVersion.getMaxSource().isLargerishThan(CK3Version)) {
							Log(LogLevel::Error) << "Converter requires a maximum save from v" << converterVersion.
									getMaxSource().toShortString();
							throw std::runtime_error("Savegame vs converter version mismatch!");
						}
					});
	...
	registerKeyword("landed_titles",
					[this](const std::string &unused, std::istream &theStream) {
						...
						Log(LogLevel::Info) << "<> Loaded " << titles.getTitles().size() << " titles: " << counter[0] <<
								"b " << counter[1] << "c " << counter[2] << "d "
								<< counter[3] << "k " << counter[4] << "e " << counter[5] << "h" << counter[6] <<
								" dynamics.";
					});
	registerKeyword("provinces",
					[this](const std::string &unused, std::istream &theStream) {
						Log(LogLevel::Info) << "-> Loading provinces.";
						provinceHoldings = ProvinceHoldings(theStream);
						Log(LogLevel::Info) << "<> Loaded " << provinceHoldings.getProvinceHoldings().size() <<
								" provinces.";
					});
	registerKeyword("living",
					[this](std::istream &theStream) {
...
void CK3::World::locatePlayerTitle(const std::shared_ptr<Configuration> &theConfiguration) {
	...
	if (playerTitle) { theConfiguration->setCraftFlagForPlayerTitle(*playerTitle); }
}
...
void CK3::World::processSave(const std::filesystem::path &saveGamePath) {
	...
	} else if (save.is_binary()) { Log(LogLevel::Error) << "Binary Save and NO META!"; }

	if (save.is_binary()) {
		Log(LogLevel::Info) << "Gamestate is binary, melting.";
		const auto &melt = save.melt();
		if (melt.has_unknown_tokens()) { Log(LogLevel::Error) << "Rakaly reports errors while melting ironman save!"; }
...
void CK3::World::verifySave(const std::filesystem::path &saveGamePath) const {
	...
	if (buffer[0] != 'S' || buffer[1] != 'A' || buffer[2] != 'V') throw std::runtime_error("Savefile of unknown type.");
...
void CK3::World::primeLaFabricaDeColor(const Configuration &theConfiguration) {
	...
	for (const auto &mod: mods) {
		if (!commonItems::DoesFolderExist(mod.path / "common/named_colors")) continue;
...
void CK3::World::loadCharacterTraits(const Configuration &theConfiguration) {
	...
	for (const auto &mod: mods) {
		if (!commonItems::DoesFolderExist(mod.path / "common/traits")) continue;
...
void CK3::World::loadHouseNames(const Configuration &theConfiguration) {
	...
	for (const auto &mod: mods) {
		if (!commonItems::DoesFolderExist(mod.path / "common/dynasty_houses")) continue;
...
void CK3::World::flagCelestialEmpire() {
	...
	if (h_china->getAlteredName())
		Log(LogLevel::Info) << "<> Proper China exists: " << h_china->getDisplayName() << " (" << *h_china->
				getAlteredName()
				<< "). All is well in the Middle Kingdom.";
	else Log(LogLevel::Info) << "<> Proper China exists: " << h_china->getDisplayName() << " as h_china.";
}

void CK3::World::flagHREProvinces(const Configuration &theConfiguration) {
	std::string hreTitleStr;
	switch (theConfiguration.getHRE()) {
		case Configuration::I_AM_HRE::HRE: hreTitleStr = "e_hre";
			break;
		case Configuration::I_AM_HRE::BYZANTIUM: hreTitleStr = "e_byzantium";
			break;
		case Configuration::I_AM_HRE::ROME: hreTitleStr = "e_roman_empire";
			break;
		case Configuration::I_AM_HRE::CUSTOM: hreTitleStr = iAmHreMapper.getHRE();
			break;
		case Configuration::I_AM_HRE::NONE: Log(LogLevel::Info) <<
					">< HRE Provinces not available due to configuration disabling HRE Mechanics.";
			return;
	}
...
void CK3::World::checkForIslam() {
	for (const auto &county: countyDetails.getCountyDetails() | std::views::values) {
		if (!county->getFaith().second) continue;
		if (!county->getFaith().second->getReligion().second) continue;
...
void CK3::World::shatterHRE(const Configuration &theConfiguration) const {
	...
	// "Emperor", in this context, is not a person but the resulting primary duchy/kingdom title of said person.
	...
	for (const auto &vassal: hreTitle->second->getDFVassals()) {
		if (vassal.second->getLevel() == LEVEL::DUCHY || vassal.second->getLevel() ==
			LEVEL::COUNTY) { hreMembers.insert(vassal); } else if (vassal.second->getLevel() == LEVEL::KINGDOM) {
			if (vassal.second->getName() == "k_papal_state" || vassal.second->getName() == "k_orthodox" ||
				theConfiguration.getShatterHRELevel() == Configuration::SHATTER_HRE_LEVEL::KINGDOM)
			// hard override for special HRE members
			{
				hreMembers.insert(vassal);
			} else {
				for (const auto &vassalvassal: vassal.second->getDFVassals()) { hreMembers.insert(vassalvassal); }
	...
	for (const auto &brick: brickList) brick.second->brickTitle();

	// Locating HRE emperor. Unlike CK2, we'll using first non-hreTitle non-landless title from hreHolder's domain.
	if (!hreHolder->second->getCharacterDomain()) throw std::runtime_error("HREmperor has no Character Domain!");
	...
	if (!emperorSet) Log(LogLevel::Warning) << "Couldn't flag His HREship as emperor does not own any viable titles!";
	...
	for (const auto &member: hreMembers) {
		...
		// This fill free emperor's holdings as well. We'll reintegrate them immediately after this.
	}
	...
	for (const auto &afflictedPerson: brickedPeople) {
		...
		for (const auto &holderTitle: holderDomain) {
			...
			if (!holderTitle.second->getDFLiege() && holderTitle.second->getDJLiege() && holderTitle.second->
				getDJLiege()->second->getHolder() &&
				holderTitle.second->getDJLiege()->second->getHolder()->first == afflictedPerson.first && holderTitles.
				count(holderTitle.first)) {
				// fix this title.
...
void CK3::World::shatterEmpires(const Configuration &theConfiguration) const {
	...
	switch (theConfiguration.getShatterLevel()) {
		case Configuration::SHATTER_LEVEL::KINGDOM: shatterKingdoms = false;
			break;
		case Configuration::SHATTER_LEVEL::DUTCHY: shatterKingdoms = true;
			break;
	}
	const auto &allTitles = titles.getTitles();
	...
	for (const auto &empire: allTitles) {
		...
		for (const auto &vassal: empire.second->getDFVassals()) {
			...
			if (vassal.second->getLevel() == LEVEL::DUCHY || vassal.second->getLevel() ==
				LEVEL::COUNTY) { members.insert(vassal); } else if (vassal.second->getLevel() == LEVEL::KINGDOM) {
				if (shatterKingdoms && vassal.second->getName() != "k_papal_state" && vassal.second->getName() !=
					...
					} else { brickedPeople.insert(*vassal.second->getHolder()); }
					brickList.insert(vassal);
				} else {
		...
		for (const auto &member: members) { member.second->grantIndependence(); }
		...
		for (const auto &afflictedPerson: brickedPeople) {
			if (!afflictedPerson.second) {
				...
				Log(LogLevel::Warning) << "Character " << afflictedPerson.first <<
						" has no link to domain! Cannot fix them.";
				continue;
			} else if (afflictedPerson.second->getCharacterDomain()->getDomain().empty()) { continue; }
			...
			for (const auto &holderTitle: holderDomain) {
				...
				if (!holderTitle.second->getDFLiege() && holderTitle.second->getDJLiege() && holderTitle.second->
					getDJLiege()->second->getHolder() &&
					holderTitle.second->getDJLiege()->second->getHolder()->first == afflictedPerson.first &&
...
void CK3::World::filterIndependentTitles() {
	...
	for (const auto &title: allTitles) {
		if (!title.second->getHolder()) continue; // don't bother with titles without holders.
		if (!title.second->getDFLiege()) {
			// this is a potential indep.
			potentialIndeps.insert(title);
		}
		if (title.second->getDFLiege() && !title.second->getDFLiege()->second->getHolder())
		// yes, we can have a dfliege that's destroyed, apparently.
		{
	...
	for (const auto &title: allTitles) {
		if (title.second->getHolder()) {
			if (title.second->getLevel() == LEVEL::COUNTY) countyHolders.insert(title.second->getHolder()->first);
...
void CK3::World::splitVassals(const Configuration &theConfiguration) {
	...
	for (const auto &title: independentTitles) {
		if (title.second->isThePope()) continue; // Not touching the pope.
		// let's not split hordes or tribals. <- TODO: Add horde here once some DLC drops.
		if (title.second->getHolder()->second->getCharacterDomain()->getGovernment() == "tribal_government") continue;
		auto  relevantVassals = 0;
		LEVEL relevantVassalLevel;
		if (title.second->getLevel() == LEVEL::EMPIRE) relevantVassalLevel = LEVEL::KINGDOM;
		else if (title.second->getLevel() == LEVEL::KINGDOM) relevantVassalLevel = LEVEL::DUCHY;
		else continue; // Not splitting off counties.
		for (const auto &vassal: title.second->getDFVassals()) {
			if (vassal.second->getLevel() != relevantVassalLevel) continue; // they are not relevant
			if (vassal.second->coalesceDFCounties().empty()) continue;      // no land, not relevant
			relevantVassals++;
		}
		...
		for (const auto &vassal: title.second->getDFVassals()) {
			if (vassal.second->getLevel() != relevantVassalLevel) continue; // they are not relevant
			if (vassal.second->getHolder()->first == title.second->getHolder()->first) continue;
			// Not splitting our own land.
...
void CK3::World::gatherCourtierNames() {
	...
	for (const auto &character: characters.getCharacters()) {
		...
		if (!character.second->isCouncilor() && !character.second->getEmployer()) continue;
		...
		if (character.second->isCouncilor()) {
			if (character.second->getEmployer() && character.second->getEmployer()->second) {
				// easiest case.
				holderCourtiers[character.second->getEmployer()->first].insert(
					std::pair(character.second->getName(), !character.second->isFemale()));
				holderCouncilors[character.second->getEmployer()->first].insert(character);
			} else if (character.second->getCharacterDomain() && !character.second->getCharacterDomain()->getDomain().
																			empty()) {
				...
				if (!characterPrimaryTitle.second) continue; // corruption
				const auto &liegeTitle = characterPrimaryTitle.second->getDFLiege();
				if (!liegeTitle || liegeTitle->second) continue;
				// I dislike this character. I think it is time he was let go.
				const auto &liege = liegeTitle->second->getHolder();
				if (!liege || !liege->second) continue; // Or maybe we should fire his liege.
				holderCourtiers[liege->first].insert(std::pair(character.second->getName(),
															   character.second->isFemale()));
	...
	Log(LogLevel::Info) << "<> " << counter << " people gathered for interrogation. " << counterAdvisors <<
			" were detained.";
}
...
void CK3::World::filterLandlessTitles() {
	...
	for (const auto &title: independentTitles) {
		if (title.second->getOwnedDFCounties().empty()) { titlesForDisposal.insert(title.first); }
	}
...
void CK3::World::setElectors() {
	...
	for (const auto &title: independentTitles) {
		...
		if (title.second->isHREEmperor()) { hreHolder = *title.second->getHolder(); }
	}
	...
	for (auto &elector: electors) {
		if (counter >= 7) break; // We had enough.
		...
		if (elector.second->getCharacterDomain() && !elector.second->getCharacterDomain()->getDomain().empty()) {
			for (const auto &electorTitle: elector.second->getCharacterDomain()->getDomain()) {
				...
				if (regItr->second.count(electorTitle.second->getName()) && !electorTitle.second->getOwnedDFCounties().
					empty()) {
