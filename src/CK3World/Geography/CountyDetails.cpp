...
void CK3::CountyDetails::registerKeys() {
	registerRegex(R"(c_[A-Za-z0-9_\-\']+)",
				  [this](const std::string &countyName, std::istream &theStream) {
					  auto newCounty = std::make_shared<CountyDetail>(theStream);
					  countyDetails.insert(std::pair(countyName, newCounty));
				  });
	registerKeyword("counties",
					[this](const std::string &countyName, std::istream &theStream) {
	...
	for (const auto &countyDetail: countyDetails) {
		...
		if (cultureDataItr != cultureData.end()) {
			...
			throw std::runtime_error(
				"County " + countyDetail.first + " has culture " + std::to_string(
					countyDetail.second->getCulture().first) + " which has no definition!");
		}
	}
...
void CK3::CountyDetails::linkFaiths(const Faiths &faiths) {
	...
	for (const auto &countyDetail: countyDetails) {
		...
		if (faithDataItr != faithData.end()) {
			...
			throw std::runtime_error(
				"County " + countyDetail.first + " has faith " + std::to_string(countyDetail.second->getFaith().first) +
