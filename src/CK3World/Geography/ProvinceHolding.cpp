...
void CK3::ProvinceHolding::registerKeys() {
	...
	registerKeyword("special_building_type",
					[this](const std::string &unused, std::istream &theStream) {
						specialBuilding = commonItems::singleString(theStream).getString();
					});
	registerKeyword("buildings",
					[this](const std::string &unused, std::istream &theStream) {
						...
						for (const auto &blob: buildingBlobs) {
							...
							if (!newBuilding.empty()) buildings.insert(newBuilding);
						}
					});
...
int CK3::ProvinceHolding::countBuildings() const {
	...
	for (const auto &building: buildings) {
		...
		try { totalBuildings += std::stoi(buildingString); } catch (std::exception &) {
			Log(LogLevel::Warning) << "Province building stoi fail: " << buildingString << " from " << building;
			totalBuildings++;
		}
	}

	if (specialBuilding.empty()) return totalBuildings;
	else return totalBuildings + 1;
}