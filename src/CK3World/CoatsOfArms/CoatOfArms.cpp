...
void CK3::CoatOfArms::registerKeys() {
	...
	registerKeyword("color1",
					[this](const std::string &unused, std::istream &theStream) {
						try { color1 = laFabricaDeColor.getColor(theStream); } catch (std::exception &e) {
							Log(LogLevel::Warning) << e.what() << " - sidestepping with black.";
							color1 = commonItems::Color(std::array < int, 3 > {
								0, 0, 0
							}
							)
							;
	...
	registerKeyword("color2",
					[this](const std::string &unused, std::istream &theStream) {
						try { color2 = laFabricaDeColor.getColor(theStream); } catch (std::exception &e) {
							Log(LogLevel::Warning) << e.what() << " - sidestepping with black.";
							color2 = commonItems::Color(std::array < int, 3 > {
								0, 0, 0
							}
							)
							;
	...
	registerKeyword("color3",
					[this](const std::string &unused, std::istream &theStream) {
						try { color3 = laFabricaDeColor.getColor(theStream); } catch (std::exception &e) {
							Log(LogLevel::Warning) << e.what() << " - sidestepping with black.";
							color3 = commonItems::Color(std::array < int, 3 > {
								0, 0, 0
							}
							)
							;
	...
	registerKeyword("textured_emblem",
					[this](const std::string &unused, std::istream &theStream) {
						texturedEmblems.emplace_back(Emblem(theStream));
					});
	registerKeyword("colored_emblem",
					[this](const std::string &unused, std::istream &theStream) {
	...
	registerKeyword("instance",
					[this](const std::string &unused, std::istream &theStream) {
						...
						if (instance.getOffset().empty()) instance.defaultOffset();
