...
void CK3::Title::registerKeys() {
	...
	nameParser.registerKeyword("title_history_names",
							   [this](const std::string &unused, std::istream &theStream) {
								   ...
								   for (const auto &theBlob: nameList) {
									   ...
									   if (!nameBlob.getDisplayName().empty() && nameBlob.getCreationDate() >
										   latestDate) {
	...
	registerKeyword("title_name_data",
					[this](const std::string &unused, std::istream &theStream) { nameParser.parseStream(theStream); });
	registerKeyword("key",
					[this](const std::string &unused, std::istream &theStream) {
	...
	registerKeyword("claim",
					[this](const std::string &unused, std::istream &theStream) {
						for (auto claimantID: commonItems::llongList(theStream).getLlongs()) claimants.insert(
							std::make_pair(claimantID, nullptr));
					});
	registerKeyword("history_government",
					[this](const std::string &unused, std::istream &theStream) {
						historyGovernment = commonItems::singleString(theStream).getString();
					});
	registerKeyword("theocratic_lease",
					[this](const std::string &unused, std::istream &theStream) {
						theocraticLease = commonItems::singleString(theStream).getString() == "yes";
					});
	registerKeyword("capital_barony",
					[this](const std::string &unused, std::istream &theStream) {
						cCapitalBarony = commonItems::singleString(theStream).getString() == "yes";
					});
	registerKeyword("duchy_capital_barony",
					[this](const std::string &unused, std::istream &theStream) {
						dCapitalBarony = commonItems::singleString(theStream).getString() == "yes";
					});
	registerKeyword("capital",
					[this](const std::string &unused, std::istream &theStream) {
						capital = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
					});
	registerKeyword("de_facto_liege",
					[this](const std::string &unused, std::istream &theStream) {
						dfLiege = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
					});
	registerKeyword("de_jure_liege",
					[this](const std::string &unused, std::istream &theStream) {
						djLiege = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
					});
	registerKeyword("de_jure_vassals",
					[this](const std::string &unused, std::istream &theStream) {
						for (auto vassalID: commonItems::llongList(theStream).getLlongs()) djVassals.insert(
							std::make_pair(vassalID, nullptr));
					});
	registerKeyword("heir",
					[this](const std::string &unused, std::istream &theStream) {
						for (auto heirID: commonItems::llongList(theStream).getLlongs()) heirs.emplace_back(
							std::make_pair(heirID, nullptr));
					});
	registerKeyword("laws",
					[this](const std::string &unused, std::istream &theStream) {
						const auto &theLaws = commonItems::stringList(theStream).getStrings();
						laws                = std::set(theLaws.begin(), theLaws.end());
					});
	registerKeyword("holder",
					[this](const std::string &unused, std::istream &theStream) {
						holder = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
					});
	registerKeyword("renamed",
					[this](std::istream &theStream) { renamed = commonItems::getString(theStream) == "yes"; });
	registerKeyword("coat_of_arms_id",
					[this](const std::string &unused, std::istream &theStream) {
						coa = std::pair(commonItems::singleLlong(theStream).getLlong(), nullptr);
					});
	registerKeyword("succession_election",
					[this](const std::string &unused, std::istream &theStream) {
						const auto newTitle = Title(theStream, 0);
						electors            = newTitle.getElectors();
					});
	registerKeyword("electors",
					[this](const std::string &unused, std::istream &theStream) {
						for (auto electorID: commonItems::intList(theStream).getInts()) electors.insert(
							std::pair(electorID, nullptr));
					});
	registerKeyword("landless",
					[this](const std::string &unused, std::istream &theStream) {
	...
	registerRegex(R"(\d+.\d+.\d+)",
				  [this](const std::string &unused, std::istream &theStream) {
					  ...
					  if (questionableItem.find('{') == std::string::npos) {
						  try {
							  previousHolders.
									  emplace_back(std::pair(commonItems::singleLlong(tempStream).getLlong(), nullptr));
						  } catch (std::exception &) {
...
void CK3::Title::cleanUpDisplayName() {
	...
	while (start != std::string::npos) {
		if ((!silence || silenceOverRide) && start != 0) {
			...
			if (displayName.length() >= start + 3) displayName = displayName.substr(start + 3, displayName.length());
			else break;
			silence         = true;
			...
			if (displayName.length() >= start + 2) displayName = displayName.substr(start + 2, displayName.length());
			else break;
			braceDepth--;
			if (braceDepth <= 0) {
				silence    = false;
			...
			silence         = true;
			silenceOverRide = false;
			if (displayName.length() >= start + 1) displayName = displayName.substr(start + 1, displayName.length());
			else break;
			braceDepth++;
			start = displayName.find('\x15');
		}
...
int CK3::Title::flagDeJureHREProvinces() {
	...
	for (const auto &deJureVassal: djVassals) { counter += deJureVassal.second->flagDeJureHREProvinces(); }
	for (const auto &deFactoVassal: dfVassals) { counter += deFactoVassal.second->flagDeJureHREProvinces(); }
	if (!clay) // We really need the county clay to confirm we're a landful proper county.
		return counter;
	if (!clay->getCounty() || getLevel() != LEVEL::COUNTY) return counter;
...
void CK3::Title::brickTitle() {
	...
	for (const auto &vassal: dfVassals)
		if (vassal.second) independentList.emplace_back(vassal.second);
		else Log(LogLevel::Warning) << "Granting independence to " << vassal.first << " that's not linked!";

	for (const auto &independentVassal: independentList) independentVassal->grantIndependence();
...
void CK3::Title::dropTitleFromDFVassals(long long titleID) {
	...
	if (dfvItr != dfVassals.end()) dfVassals.erase(dfvItr);
	else Log(LogLevel::Warning) << "dropping vassal " << titleID << " that doesn't exist!";
}
...
CK3::LEVEL CK3::Title::getLevel() const {
	...
	if (name.starts_with("b_")) return LEVEL::BARONY;
	if (name.starts_with("c_")) return LEVEL::COUNTY;
	if (name.starts_with("d_")) return LEVEL::DUCHY;
	if (name.starts_with("k_")) return LEVEL::KINGDOM;
	if (name.starts_with("e_")) return LEVEL::EMPIRE;
	if (name.starts_with("h_")) return LEVEL::HEGEMONY;
...
void CK3::Title::relinkDeFactoVassals() {
	...
	if (name.starts_with("c_") || name.starts_with("b_")) return; // don't bother with counties and below.

	std::set<long long> dropList;

	for (const auto &[dfVassalID, dfVassal]: dfVassals) {
		if (!dfVassal) continue;

		if (dfVassal->areDFLiegeAndDJLiegeSet() && !dfVassal->areDFLiegeAndDJLiegeSame())
		// We have different df and dj lieges there.
		{
			...
			if (doesHolderHaveCharacterDomain()) {
				...
				if (titleNameCache.contains(dfVassal->getDFLiege()->second->getName()) && titleNameCache.contains(
					dfVassal->getDJLiege()->second->getName())) {
				...
				if (!isHolderSet()) Log(LogLevel::Warning) << name << " has no holder but has vassals?";
				else if (!isHolderLinked()) Log(LogLevel::Warning) << name << "'s holder is not linked up!";
				else Log(LogLevel::Warning) << name << "' holder has no domain but has vassals?";
			}
		}
	}

	for (auto dropVassalID: dropList) dropTitleFromDFVassals(dropVassalID);
}

bool CK3::Title::areDFLiegeAndDJLiegeSet() const { return dfLiege && djLiege; }

bool CK3::Title::areDFLiegeAndDJLiegeSame() const {
	if (!areDFLiegeAndDJLiegeSet()) return false; // Can't check if they're not set.
...
std::set<std::string> CK3::Title::getTitleNamesFromHolderDomain() const {
	...
	if (!doesHolderHaveCharacterDomain()) return toReturn;

	const auto holderTitles = holder->second->getCharacterDomain()->getDomain();

	for (const auto &[domainTitleID, domainTitle]: holderTitles)
		if (domainTitle) toReturn.insert(domainTitle->getName());

	return toReturn;
}

std::optional<std::string> CK3::Title::getAlteredName() const {
	if (alteredName.empty()) return std::nullopt;

	if (alteredName == name) return std::nullopt;
