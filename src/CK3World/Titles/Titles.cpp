...
CK3::Titles::Titles(std::istream &theStream) {
	...
	if (!dynamicTitleRanks.empty()) transcribeDynamicRanks();
}

void CK3::Titles::registerKeys() {
	registerKeyword("dynamic_templates",
	...
	registerRegex(R"(\d+)",
				  [this](const std::string &ID, std::istream &theStream) {
					  ...
					  if (titleBlob.find('{') != std::string::npos) {
						  ...
						  try {
							  ...
							  if (!newTitle->getName().empty()) {
								  ...
								  if (newTitle->getName().find("b_") == 0) ++titleCounter[0];
								  else if (newTitle->getName().find("c_") == 0) ++titleCounter[1];
								  else if (newTitle->getName().find("d_") == 0) ++titleCounter[2];
								  else if (newTitle->getName().find("k_") == 0) ++titleCounter[3];
								  else if (newTitle->getName().find("e_") == 0) ++titleCounter[4];
								  else if (newTitle->getName().find("h_") == 0) ++titleCounter[5];
								  else ++titleCounter[6]; // x_x_, x_mc_ and the rest.
							  }
...
void CK3::Titles::transcribeDynamicRanks() {
	...
	for (const auto &[key, rank]: dynamicTitleRanks) {
		if (!titles.contains(key)) continue; // Yay?
		if (!titles.at(key)) continue;       // Huh?

		// There will not be a dynamic county or barony.
		if (rank == "duchy") titles.at(key)->setDynamicLevel(LEVEL::DUCHY);
...
void CK3::Titles::linkCoats(const CoatsOfArms &coats) {
	...
	for (const auto &title: titles) {
		if (!title.second->getCoA()) continue;
		...
		if (coatDataItr != coatData.end()) {
			...
			Log(LogLevel::Warning) << "Title " + title.first + " has CoA " + std::to_string(
						title.second->getCoA()->first) +
...
void CK3::Titles::linkTitles() {
	...
	for (const auto &title: titles) IDCache.insert(std::pair(title.second->getID(), title.second));

	// We'll also be needing a defacto vassal registry for later.
	std::map<long long, std::map<long long, std::shared_ptr<Title> > > dfvRegistry;
	// map<liegeTitleID, map<vassalTitleID, vassalTitle>>
	...
	for (const auto &title: titles) {
		...
		if (title.second->getDFLiege()) {
			...
			if (holderlessTitles.count(title.second->getDFLiege()->first)) {
				// fix it immediately.
				Log(LogLevel::Warning) << "Title " + title.first + " has defacto liege " + std::to_string(
							title.second->getDFLiege()->first) +
				...
				if (cacheItr != IDCache.end()) {
					...
					dfvRegistry[title.second->getDFLiege()->first].insert(
						std::pair(title.second->getID(), title.second));
					++DFLcounter;
				} else {
		...
		if (title.second->getDJLiege()) {
			...
			if (cacheItr != IDCache.end()) {
				...
				Log(LogLevel::Warning) << "Title " << title.first << " has DEJURE >>LIEGE<< " << std::to_string(
							title.second->getDJLiege()->first)
						<< " which has no definition! This is not amusing. Rerouting.";
				if (title.second->getDFLiege()) {
					if (const auto &dfLiegeItr = IDCache.find(title.second->getDFLiege()->first); dfLiegeItr != IDCache.
						end()) {
						...
						Log(LogLevel::Warning) << "Rerouted " << title.first << " to dejure liege " << dfLiegeItr->
								second->getName();
					} else {
						Log(LogLevel::Warning) << "Title " << title.first <<
								" also has a missing defacto liege. Your save is likely very broken. Rerouting to void.";
						title.second->resetDFLiege();
		...
		if (!title.second->getDJVassals().empty()) {
			...
			for (const auto &dJVassal: title.second->getDJVassals()) {
				...
				if (cacheItr != IDCache.end()) {
					...
					Log(LogLevel::Warning) << "Title " << title.first << " has DEJURE vassal " << std::to_string(
								dJVassal.first)
	...
	for (const auto &update: updateMap) { IDCache[update.first]->addDFVassals(update.second); }

	Log(LogLevel::Info) << "<> " << DFLcounter << " defacto lieges, " << DJLcounter << " dejure lieges, " << DFVcounter
			<< " defacto vassals, " << DJVcounter
			<< " dejure vassals updated.";
}

void CK3::Titles::relinkDeFactoVassals() { for (const auto &title: titles) title.second->relinkDeFactoVassals(); }
...
void CK3::Titles::linkCharacters(const Characters &characters) {
	...
	for (const auto &title: titles) {
		...
		if (title.second->getHolder()) {
			...
			if (characterDataItr != characterData.end()) {
				...
				Log(LogLevel::Error) << "For heaven's sake, title " + title.first + " has holder " + std::to_string(
							title.second->getHolder()->first) +
				...
				if (title.second->getDFLiege()) {
					...
					for (const auto &[titleName, theTitle]: titles)
						if (theTitle->getID() == title.second->getDFLiege()->first) {
							if (theTitle->isHolderSet()) replacementHolder = theTitle->getHolder()->first;
							break;
						}

					if (replacementHolder > 0)
						if (const auto &replacementHolderItr = characterData.find(replacementHolder);
							replacementHolderItr != characterData.end()) {
	...
	Log(LogLevel::Info) << "<> " << holderCounter << " holders, " << claimantCounter << " claimants, " << heirCounter <<
			" heirs, " << exCounter
