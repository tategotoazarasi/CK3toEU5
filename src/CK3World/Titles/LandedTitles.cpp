...
void CK3::LandedTitles::registerKeys() {
	...
	registerKeyword("color",
					[this](const std::string &unused, std::istream &theStream) {
						try { color = laFabricaDeColor.getColor(theStream); } catch (std::exception &e) {
							Log(LogLevel::Error) << "Color loading failure: " << e.what() <<
									" ... Replacing with grey.";
							color = commonItems::Color(std::array < int, 3 > {
								100, 100, 100
							}
							)
							;
	...
	registerKeyword("can_be_named_after_dynasty",
					[this](const std::string &unused, std::istream &theStream) {
...
void CK3::LandedTitles::linkProvinceHoldings(const ProvinceHoldings &provinceHoldings) {
	...
	for (const auto &landedTitle: foundTitles) {
		...
		if (provinceDataItr != provinceData.end()) {
			...
			throw std::runtime_error(
				"Landed title " + landedTitle.first + " has province holding " + std::to_string(
					landedTitle.second->getProvince()->first) +
...
void CK3::LandedTitles::linkCountyDetails(const CountyDetails &countyDetails) {
	...
	for (const auto &landedTitle: foundTitles) {
		if (landedTitle.first.find("c_") != 0) continue;
		...
		if (countyDataItr != countyData.end()) {
			...
			Log(LogLevel::Warning) << "Landed title " << landedTitle.first <<
					" has no definition in counties={} section of the save game!";
		}
	}
	Log(LogLevel::Info) << "<> " << counter << " landed titles updated.";
}
...
void CK3::LandedTitles::linkTitles(const Titles &titles) {
	...
	for (const auto &landedTitle: foundTitles) {
		...
		if (const auto &titleDataItr = titleData.find(theCapital->first); titleDataItr != titleData.end()) {
			...
			Log(LogLevel::Error) << "Landed title " + landedTitle.first + " has a capital " + theCapital->first +
					" which has no definition!";
		}
	}
