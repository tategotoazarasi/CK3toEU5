...
namespace CK3 {
	...
	public:
		...
		void loadProvinceHolding(const std::pair<int, std::shared_ptr<ProvinceHolding> > &provinceHolding) {
			province = provinceHolding;
		}

		void loadCountyDetails(const std::pair<std::string, std::shared_ptr<CountyDetail> > &countyDetail) {
			county = countyDetail;
		}
