...
void CK3::Confederations::registerKeys() {
	databaseParser.registerRegex(R"(\d+)",
								 [this](const std::string &confederationID, std::istream &theStream) {
									 ...
									 if (questionableItem.find('{') != std::string::npos) {
										 try {
											 auto newConfederation = std::make_shared<Confederation>(
												 tempStream,
												 std::stoll(confederationID));
											 confederations.
													 insert(std::pair(newConfederation->getID(), newConfederation));
										 } catch (std::exception &) {
											 Log(LogLevel::Warning) << "Invalid confederation blob: " <<
													 questionableItem << " at ID: " << confederationID;
										 }
									 }
								 });
	databaseParser.registerRegex(commonItems::catchallRegex, commonItems::ignoreItem);

	registerKeyword("database", [this](std::istream &theStream) { databaseParser.parseStream(theStream); });
...
void CK3::Confederations::linkCoats(const CoatsOfArms &coats) {
	...
	for (const auto &confederation: confederations) {
		if (!confederation.second->getCoat()) continue;
		...
		if (coatDataItr != coatData.end()) {
			...
			Log(LogLevel::Warning) << "Confederation " << confederation.second->getName() << " has CoA " <<
					std::to_string(confederation.second->getCoat()->first)
